 ...
 * - GET /api/admin/users - 查询用户列表（需管理员权限）
 * - GET /api/admin/users/{id} - 获取用户详情（需管理员权限）
 * - PATCH /api/admin/users/{id} - 更新用户信息（需管理员权限）
 * - DELETE /api/admin/users/{id} - 删除用户（需管理员权限）
 * - PATCH /api/admin/users/{id}/role - 修改用户角色（需组织者权限）
 * - PATCH /api/admin/users/{id}/status - 修改用户状态（需管理员权限）
 * - PATCH /api/admin/users/{id}/password - 重置用户密码（需管理员权限）
 */
@RestController
...
public class AdminController {
        ...
        public ResponseEntity<ApiResponse<String>> changeUserRole(
                        @Parameter(description = "用户ID", required = true) @PathVariable Long userId,
                        @Parameter(description = "新角色：1-组织者，2-管理员，3-普通用户", required = true) @RequestParam Integer role) {
        ...
        public ResponseEntity<ApiResponse<String>> changeUserStatus(
                        @Parameter(description = "用户ID", required = true) @PathVariable Long userId,
                        @Parameter(description = "新状态：0-未激活，1-正常，2-禁用", required = true) @RequestParam Integer status) {
